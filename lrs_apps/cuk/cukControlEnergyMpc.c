...
#define DT      ((float)(1.0f/100e3))

#define ALPHA   ((float)1e6)

#define CO      ((float)330e-6)
...
int32_t cukControlEnergyMpcRun(void *meas, int32_t nmeas, void *refs, int32_t nrefs, void *outputs, int32_t nmaxoutputs){
    ...
    /* Delay compensation */
    y = y + DT * y_dot + ALPHA * DT * DT / 2.0f * v_1;
    y_dot = y_dot + ALPHA * DT * v_1;

    /* Determine bounds for dv */
    dv_min_v = hwm->vi_dc / CUK_CONFIG_L_IN * (hwm->vi_dc - x3) / ALPHA - v_1;
    dv_max_v = hwm->vi_dc * hwm->vi_dc / CUK_CONFIG_L_IN / ALPHA - v_1;

    dv_min_z2 = (IL_MIN * hwm->vi_dc - hwm->vo * hwm->io - 2.0f * y_dot + y_dot_1) / (ALPHA * DT);
    dv_max_z2 = (IL_MAX * hwm->vi_dc - hwm->vo * hwm->io - 2.0f * y_dot + y_dot_1) / (ALPHA * DT);

    dv_min = dv_min_v > dv_min_z2 ? dv_min_v : dv_min_z2;
    dv_max = dv_max_v < dv_max_z2 ? dv_max_v : dv_max_z2;

    /* Optimization */
    dv = - Ky * (y - yr) - K_dz_1 * (y - y_1) - K_dz_2 * (y_dot - y_dot_1);
    ...
    duty = 1.0f - 1.0f / x3 * (hwm->vi_dc - CUK_CONFIG_L_IN / hwm->vi_dc * ALPHA * v);
...
int32_t cukControlEnergyMpcFirstEntry(void *meas, int32_t nmeas,
    ...
    // y_dot = p_in - p_out;
    //
    // //v_init = hwm->vi_dc / CUK_CONFIG_L_IN * (hwm->vi_dc - (1.0f - o->u) * x3) / ALPHA;
    //
    // v_1 = 0;
    // y_1 = y;
    // y_dot_1 = y_dot;
